...
check_root(){
	[[ $EUID != 0 ]] && echo -e "${Error} NO ROOT PLZ USE ROOT${Green_background_prefix} sudo su ${Font_color_suffix}来获取临时ROOT权限（执行后会提示输入当前账号的密码）。" && exit 1
}
check_sys(){
...
SSR_installation_status(){
	[[ ! -e ${config_user_file} ]] && echo -e "${Error} NO ShadowsocksR DATA, PLZ CHECK !" && exit 1
	[[ ! -e ${ssr_folder} ]] && echo -e "${Error} NO ShadowsocksR FOLDER，PLZ CHECK !" && exit 1
}
Server_Speeder_installation_status(){
	[[ ! -e ${Server_Speeder_file} ]] && echo -e "${Error} DIDNT INSTALL(Server Speeder)，PLZ CHEAK !" && exit 1
}
LotServer_installation_status(){
	[[ ! -e ${LotServer_file} ]] && echo -e "${Error} DIDN LotServer，PLZ CHECK !" && exit 1
}
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		echo -e "${Error} NO BBR CODE，NOW DOWNLODING..."
		cd "${file}"
		if ! wget -N --no-check-certificate https://raw.githubusercontent.com/ToyoDAdoubi/doubi/master/bbr.sh; then
			echo -e "${Error} BBR ERROR CAN NOT DOWNLOAD !" && exit 1
		else
			echo -e "${Info} BBR SUCCESS !"
			chmod +x bbr.sh
		fi
	fi
}
...
Get_User(){
	[[ ! -e ${jq_file} ]] && echo -e "${Error} JQ DID NOT EXIST !" && exit 1
...
View_User(){
	...
	if [[ -z "${now_mode}" ]]; then
		...
		echo -e " GARBLE\t    : ${Red_font_prefix}${obfs}${Font_color_suffix}"
		echo -e " Device limit : ${Green_font_prefix}${protocol_param}${Font_color_suffix}"
		echo -e " SPEED LIMIT : ${Green_font_prefix}${speed_limit_per_con} KB/S${Font_color_suffix}"
		echo -e " ALL SPEED LIMIT : ${Green_font_prefix}${speed_limit_per_user} KB/S${Font_color_suffix}"
		echo -e "${ss_link}"
		echo -e "${ssr_link}"
		echo -e " ${Green_font_prefix} 提示: ${Font_color_suffix}
 OPEN URL IN CHROME YOU CAN SEE THE QR CODE。
 PROTOCOL IS BEHIND THE GARBLE[ _compatible ]，
Refers to compatibility with the original  PROTOCOL/ GARBLE。"
		echo && echo "==================================================="
	else
		user_total=`${jq_file} '.port_password' ${config_user_file} | sed '$d' | sed "1d" | wc -l`
		[[ ${user_total} = "0" ]] && echo -e "${Error} NO MUTI PORT USERS!" && exit 1
		clear && echo "===================================================" && echo
		echo -e " ShadowsocksR DATA：" && echo
		echo -e " I  P\t    : ${Green_font_prefix}${ip}${Font_color_suffix}"
		echo -e " ENCRYPT\t    : ${Green_font_prefix}${method}${Font_color_suffix}"
		echo -e " PROTOCOL\t    : ${Red_font_prefix}${protocol}${Font_color_suffix}"
		echo -e " GARBLE\t    : ${Red_font_prefix}${obfs}${Font_color_suffix}"
		echo -e " LIMIT DEVICE : ${Green_font_prefix}${protocol_param}${Font_color_suffix}"
		echo -e " SPEED LIMIT : ${Green_font_prefix}${speed_limit_per_con} KB/S${Font_color_suffix}"
		echo -e " ALL SPEED LIMIT : ${Green_font_prefix}${speed_limit_per_user} KB/S${Font_color_suffix}" && echo
...
Set_config_port(){
	while true
	do
	echo -e "SETUP ShadowsocksR PORT"
	...
	if [[ $? == 0 ]]; then
		if [[ ${ssr_port} -ge 1 ]] && [[ ${ssr_port} -le 65535 ]]; then
			...
			echo -e "${Error} ENTER RIGHT NUMBER(1-65535)"
		fi
	else
		echo -e "${Error} ENTER RIGHT NUMBER(1-65535)"
	fi
...
Set_config_password(){
	echo "SETUP ShadowsocksR PASSWORD"
...
Set_config_method(){
	echo -e "SETUP ShadowsocksR ENCRYPT
	
 ${Green_font_prefix} 1.${Font_color_suffix} none
 ${Tip} IF USE auth_chain_a ，PROTOCOL none
 ...
 ${Tip} salsa20/chacha20-* need install libsodium ，or you cannot start ShadowsocksR !" && echo
	read -e -p "(defalt: 5. aes-128-ctr):" ssr_method
...
Set_config_protocol(){
	echo -e "SETUP ShadowsocksR账号 PROTOCOL
 ...
 ${Tip} IF U USE auth_chain_a ，请加密方式选择 none，混淆随意(建议 plain)" && echo
...
3. Debian 更换内核过程中会提示 [ 是否终止卸载内核 ] ，请选择 ${Green_font_prefix} NO ${Font_color_suffix}" && echo
	...
	if [[ ${bbr_num} == "1" ]]; then
		...
		echo -e "${Error} ENTER RIGHT NUMBER(1-4)" && exit 1
	fi
}
Install_BBR(){
	[[ ${release} = "centos" ]] && echo -e "${Error} NOT SUPPORTED CentOS INSTALL BBR !" && exit 1
	BBR_installation_status
	bash "${BBR_file}"
}
Start_BBR(){
...
Other_functions(){
  ...
  ${Green_font_prefix}2.${Font_color_suffix} INSTALL (ServerSpeeder)
  ${Green_font_prefix}3.${Font_color_suffix} INSTALL LotServer
  ERROR： ServerSpeeder/LotServer/BBR NOT SUPPORT OpenVZ！
  注意： ServerSpeeder/LotServer/BBR CHOOSE ONE！
————————————
  ${Green_font_prefix}4.${Font_color_suffix} LOCK BT/PT/SPAM (iptables)
  ${Green_font_prefix}5.${Font_color_suffix} UNLOCK BT/PT/SPAM (iptables)
  ${Green_font_prefix}6.${Font_color_suffix} SWICH ShadowsocksR DATA MODE
  YOU CAN CHANGE TO PRO MODE" && echo
	...
	if [[ ${other_num} == "1" ]]; then
		...
		echo -e "${Error}  ERROR enter the right number[1-6]" && exit 1
	fi
}
# LOCK BT PT SPAM
BanBTPTSPAM(){
...
Set_config_connect_verbose_info(){
	...
	if [[ ${connect_verbose_info} = "0" ]]; then
		echo && echo -e "NOW DATA MODE: ${Green_font_prefix}SIMPLE MODE（BAD DATA）${Font_color_suffix}" && echo
		echo -e "SURE YOU WANT TO SWICH TO ${Green_font_prefix}PRO MODE CONNECT DATA+ BAD DAT${Font_color_suffix}？[y/N]"
		read -e -p "(DEFAUT: n):" connect_verbose_info_ny
		...
		echo && echo -e "NOW DATA MODE: ${Green_font_prefix}PRO MODE CONNECT DATA+ BAD DATA ${Font_color_suffix}" && echo
		echo -e "SURE YOU WANT TO SWICH TO ${Green_font_prefix}SIMPLE MODE（BAD DATA）${Font_color_suffix}？[y/N]"
		read -e -p "(DEFAUT: n):" connect_verbose_info_ny
		[[ -z "${connect_verbose_info_ny}" ]] && connect_verbose_info_ny="n"
		if [[ ${connect_verbose_info_ny} == [Yy] ]]; then
...
Update_Shell(){
	...
	echo -e "IT IS THE NEWEST VER[ ${sh_new_ver} ] ![WARNING MAY BE THERE IS SOME ERROR JUST DONT CARE]" && exit 0
}
# SHOW DATA
menu_status(){
	if [[ -e ${config_user_file} ]]; then
		check_pid
		if [[ ! -z "${PID}" ]]; then
			echo -e " NOW MODE: ${Green_font_prefix}INSTALLED${Font_color_suffix} 并 ${Green_font_prefix}STARTED${Font_color_suffix}"
		else
			echo -e " NOW MODE: ${Green_font_prefix}INSTALLED${Font_color_suffix} 但 ${Red_font_prefix}DIDNT START${Font_color_suffix}"
		fi
		now_mode=$(cat "${config_user_file}"|grep '"port_password"')
		if [[ -z "${now_mode}" ]]; then
			echo -e " NOW MODE: ${Green_font_prefix}ONE PORT${Font_color_suffix}"
		else
			echo -e " NOW MODE: ${Green_font_prefix}MUTI PORT${Font_color_suffix}"
		fi
	else
		echo -e " NOW MODE: ${Red_font_prefix}DIDNT INSTALL${Font_color_suffix}"
	fi
}
check_sys
[[ ${release} != "debian" ]] && [[ ${release} != "ubuntu" ]] && [[ ${release} != "centos" ]] && echo -e "${Error} THIS CODE CANNOT BE USE ON THIS SYSTEM ${release} !" && exit 1
echo -e "  ShadowsocksR ONE-CLICK INSTALL ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
  ---- Toyo | doub.io/ss-jc42 ----

  ${Green_font_prefix}1.${Font_color_suffix} INSTALL ShadowsocksR
  ${Green_font_prefix}2.${Font_color_suffix} UPDATE ShadowsocksR
  ${Green_font_prefix}3.${Font_color_suffix} DELETE ShadowsocksR
  ${Green_font_prefix}4.${Font_color_suffix} INSTALL libsodium(chacha20)
————————————
  ${Green_font_prefix}5.${Font_color_suffix} CHECK USER
  ${Green_font_prefix}6.${Font_color_suffix} SHOW CONNECT INFOMATION
  ${Green_font_prefix}7.${Font_color_suffix} SETUP USER
  ${Green_font_prefix}8.${Font_color_suffix} CHANGE THE VPN BY YOURSELF
...
————————————
 ...
 ${Green_font_prefix}13.${Font_color_suffix} CHECK ShadowsocksR DATA
————————————
 ${Green_font_prefix}14.${Font_color_suffix} OUTHERS
 ${Green_font_prefix}15.${Font_color_suffix} UPDATE
 "
menu_status
echo && read -e -p "ENTER NUMBER [1-15]：" num
case "$num" in
	1)
	Install_SSR
	;;
	...
	;;
	*)
	echo -e "${Error} ERROR enter the right number [1-15]"
	;;
esac
